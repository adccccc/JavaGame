...
public class Entity {
    ...
    public CcPolygon getCollisionPolyForNull() {return this.collisionPoly != null ? this.collisionPoly : new CcPolygon(this.width, this.height);}
    ...
    // 计算缩放后的绘画起点位置 (中心缩放)
    public CcVector getScreenStartPos() {return new CcVector(x + width / 2.0 * (1 - scale),  y + height / 2.0 * (1 - scale));}


    // -------------------------- 物体运动状态操控 -----------------------------------

    public void reCalcSpeed() { hSpeed += hAcceleration; vSpeed += vAcceleration; }

    public void reCalcLocation() { x += hSpeed; y += vSpeed; }
